...
@app.tool()
def create_booking(id: str, flight_no: str, arrival_time: str, departure_time: str, seat_type: str, seat_amount: int):
    """
    จองเที่ยวบินสำหรับผู้โดยสาร
    - id: รหัสผู้โดยสาร
    - flight_no: หมายเลขเที่ยวบิน เช่น TG101
    - arrival_time: เวลาถึง รูปแบบ DD-MM-YYYY HH:MM
    - departure_time: เวลาออกเดินทาง รูปแบบ DD-MM-YYYY HH:MM
    - seat_type: ประเภทที่นั่ง 'Economy' หรือ 'Business'
    - seat_amount: จำนวนที่นั่ง
    """
    new_book = airline_system.booking(id, flight_no, arrival_time, departure_time, seat_type, seat_amount)
    return {"Message": "Success", "info": new_book.get_data()}


@app.tool()
def get_booking(id: str):
    """
    ดูการจองทั้งหมดของผู้โดยสาร
    - id: รหัสผู้โดยสาร
    """
...
@app.tool()
def pay_booking(id: str, pnr: str, payment_method: str, validate_object: str):
    """
    ชำระเงินสำหรับการจองที่มีอยู่
    - id: รหัสผู้โดยสาร
    - pnr: รหัสการจอง
    - payment_method: วิธีชำระเงิน 'PayByCard' หรือ 'PayByPoint'
    - validate_object: รหัส PIN ของบัตร ถ้าใช้ PayByCard หรือเว้นว่างถ้าใช้ PayByPoint
    """
...
@app.tool()
def create_income_report(flight_no: str):
    """
    สร้างรายงานรายได้ของเที่ยวบิน
    - flight_no: หมายเลขเที่ยวบิน เช่น TG101
    """
    return {"Message": airline_system.create_income_report(flight_no)}
...
def create_account(name: str, email: str, card_pin: str, money: float, tier: str):
    ...
    - name: ชื่อ-นามสกุล
    - email: อีเมล
    - card_pin: รหัส PIN 6 หลัก
    - money: ยอดเงินในบัตร (ต้องมากกว่า 0)
    - tier: ระดับสมาชิก 'Guest', 'Silver', 'Gold', หรือ 'Platinum'
    """
    new_passenger = airline_system.create_account(name, email, card_pin, money, tier)
    return {"Message": new_passenger.get_data()}
